...
Use Automatic TTL. Once DNS resolves, set www.clatchamp.com in this repository's Settings → Pages → Custom domain, and enable Enforce HTTPS when the certificate is ready. Configuring the custom domain before DNS is ready redirects the working github.io preview to an unready domain, so it is intentionally deferred.
